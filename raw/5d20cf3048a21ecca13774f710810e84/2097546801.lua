local L0_1, L1_1, L2_1
L0_1 = require
L1_1 = "Logic.RedDotModule.Filters.BaseFilter"
L0_1(L1_1)
L0_1 = G
L0_1 = L0_1.Class
L1_1 = "ChessRogueActivityPanelFilter"
L2_1 = G
L2_1 = L2_1.BaseFilter
L0_1 = L0_1(L1_1, L2_1)
function L1_1(A0_2)
  local L1_2
  A0_2._native_filter = nil
  A0_2._cabinet_data = nil
  A0_2._activity_data = nil
  A0_2._is_activity_unlocked = false
  A0_2._is_guide_mission_finished = false
end
L0_1.ctor = L1_1
function L1_1(A0_2)
  local L1_2, L2_2
  L1_2 = A0_2._native_filter
  if L1_2 ~= nil then
    L1_2 = A0_2._native_filter
    L2_2 = L1_2
    L1_2 = L1_2.Dispose
    L1_2(L2_2)
  end
  A0_2._native_filter = nil
end
L0_1.dtor = L1_1
function L1_1(A0_2)
  local L1_2
  L1_2 = "ChessRogueActivityPanelGotoBtn"
  return L1_2
end
L0_1.get_associate_reddot = L1_1
function L1_1(A0_2)
  local L1_2, L2_2
  L2_2 = A0_2
  L1_2 = A0_2._try_get_activity_data
  L1_2(L2_2)
  L1_2 = A0_2._activity_data
  if L1_2 ~= nil then
    L2_2 = A0_2
    L1_2 = A0_2._refresh_goto_btn
    L1_2(L2_2)
  end
  L2_2 = A0_2
  L1_2 = A0_2._try_init_native_filter
  L1_2(L2_2)
  L2_2 = A0_2
  L1_2 = A0_2._refresh_unlock_cond
  L1_2(L2_2)
  L1_2 = A0_2._is_activity_unlocked
  if L1_2 then
    L1_2 = A0_2._is_guide_mission_finished
    if L1_2 then
      goto lbl_19
    end
  end
  do return end
  ::lbl_19::
  L2_2 = A0_2
  L1_2 = A0_2._refresh_cabinet_reward
  L1_2(L2_2)
  L2_2 = A0_2
  L1_2 = A0_2._refresh_main_story_reward
  L1_2(L2_2)
end
L0_1.refresh = L1_1
function L1_1(A0_2)
  local L1_2
  L1_2 = A0_2._is_activity_unlocked
  if not L1_2 then
    L1_2 = CS
    L1_2 = L1_2.RPG
    L1_2 = L1_2.Client
    L1_2 = L1_2.ChessRogueUtils
    L1_2 = L1_2.IsActivityUnlocked
    L1_2 = L1_2()
  end
  A0_2._is_activity_unlocked = L1_2
  L1_2 = A0_2._is_guide_mission_finished
  if not L1_2 then
    L1_2 = CS
    L1_2 = L1_2.RPG
    L1_2 = L1_2.Client
    L1_2 = L1_2.ChessRogueUtils
    L1_2 = L1_2.IsGuideMissionFinished
    L1_2 = L1_2()
  end
  A0_2._is_guide_mission_finished = L1_2
end
L0_1._refresh_unlock_cond = L1_1
function L1_1(A0_2)
  local L1_2, L2_2, L3_2
  L1_2 = A0_2._native_filter
  L2_2 = L1_2
  L1_2 = L1_2.UpdateMainStoryReward
  L1_2(L2_2)
  L1_2 = A0_2._native_filter
  L1_2 = L1_2.HasAnyMainStoryRewardAvailable
  if L1_2 then
    L1_2 = G
    L1_2 = L1_2.RedDotModule
    L1_2 = L1_2.Instance
    L2_2 = L1_2
    L1_2 = L1_2.update_reddot
    L3_2 = "ChessRogueMainStoryReward"
    L1_2(L2_2, L3_2)
  end
end
L0_1._refresh_main_story_reward = L1_1
function L1_1(A0_2)
  local L1_2, L2_2, L3_2, L4_2, L5_2, L6_2, L7_2, L8_2, L9_2
  L1_2 = A0_2._native_filter
  L2_2 = L1_2
  L1_2 = L1_2.UpdateCabinetReward
  L1_2(L2_2)
  L1_2 = pairs
  L2_2 = A0_2._native_filter
  L2_2 = L2_2.CabinetRewardAvailableIDs
  L1_2, L2_2, L3_2 = L1_2(L2_2)
  for L4_2, L5_2 in L1_2, L2_2, L3_2 do
    L6_2 = G
    L6_2 = L6_2.RedDotModule
    L6_2 = L6_2.Instance
    L7_2 = L6_2
    L6_2 = L6_2.update_reddot
    L8_2 = "ChessRogueCabinetReward"
    L9_2 = L5_2
    L6_2(L7_2, L8_2, L9_2)
  end
end
L0_1._refresh_cabinet_reward = L1_1
function L1_1(A0_2)
  local L1_2, L2_2, L3_2
  L1_2 = A0_2._activity_data
  L2_2 = L1_2
  L1_2 = L1_2.ShouldShowGuideMissionRedDot
  L1_2 = L1_2(L2_2)
  if not L1_2 then
    L1_2 = A0_2._activity_data
    L2_2 = L1_2
    L1_2 = L1_2.ShouldShowMainPageNotSeenRedDot
    L1_2 = L1_2(L2_2)
    if not L1_2 then
      goto lbl_17
    end
  end
  L1_2 = G
  L1_2 = L1_2.RedDotModule
  L1_2 = L1_2.Instance
  L2_2 = L1_2
  L1_2 = L1_2.update_reddot
  L3_2 = "ChessRogueActivityPanelGotoBtn"
  L1_2(L2_2, L3_2)
  ::lbl_17::
end
L0_1._refresh_goto_btn = L1_1
function L1_1(A0_2)
  local L1_2, L2_2, L3_2
  L1_2 = A0_2._activity_data
  if L1_2 ~= nil then
    return
  end
  L1_2 = CS
  L1_2 = L1_2.RPG
  L1_2 = L1_2.Client
  L1_2 = L1_2.GlobalVars
  L1_2 = L1_2.s_ModuleManager
  L1_2 = L1_2.ActivityModule
  L3_2 = L1_2
  L2_2 = L1_2.GetChessRogueActivityData
  L2_2 = L2_2(L3_2)
  A0_2._activity_data = L2_2
end
L0_1._try_get_activity_data = L1_1
function L1_1(A0_2)
  local L1_2, L2_2
  L1_2 = A0_2._native_filter
  if L1_2 ~= nil then
    return
  end
  L1_2 = CS
  L1_2 = L1_2.RPG
  L1_2 = L1_2.Client
  L1_2 = L1_2.ChessRogueRedDotFilter
  L1_2 = L1_2()
  A0_2._native_filter = L1_2
  L1_2 = A0_2._native_filter
  L2_2 = L1_2
  L1_2 = L1_2.Init
  L1_2(L2_2)
end
L0_1._try_init_native_filter = L1_1
return L0_1
